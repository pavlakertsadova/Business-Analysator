...
if mode == "Единичен анализ":
    if st.button("Стартирай Мулти-агентен анализ"):
        ...
        # 1. СИСТЕМА ЗА ПРОВЕРКА НА ФАКТИТЕ (Hallucination Check)
        st.subheader("🔍 Проверка на фактите (Guardrails)")
        # Тук Python кодът "заковава" фактите преди AI да почне да говори
        if debt > 2.0:
        ...
        # 2. МУЛТИ-АГЕНТЕН СПОР (The Debate)
        ...
        # 3. EXPLAINABLE AI (XAI) - Топлинна карта
        st.subheader("🌡️ Топлинна карта на влиянието (XAI)")
        # Логика: Определяме цветовете според тежестта на показателите
    ...
    if uploaded_file:
        ...
        # ПРЕДВАРИТЕЛНА ОБРАБОТКА (Logic Layer)
        # Изчисляваме Марж и Дълг автоматично от суровите данни
        df['Margin'] = (df['Net Income'] / (df['Total Assets'] * 0.5)) * 100  # Приблизителен марж
        ...
        if st.button("AI Ранкинг на всички компании"):
            with st.spinner("Агентите сравняват компаниите..."):
                ...
                # Изпращаме всичко на съдията за финална класация
